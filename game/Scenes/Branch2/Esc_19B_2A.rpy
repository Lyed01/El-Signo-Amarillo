label esc_19B_2A:

    scene bg estudio_scott_tarde_nublada with fade

    "El estudio está en calma. La luz gris entra por las ventanas, tiñendo los lienzos de un tono pálido y melancólico."
    "SCOTT pinta con concentración; el sonido del pincel contra el lienzo es lo único que rompe el silencio."
    "Su nueva modelo, ya se ha marchado hace un rato. El aire aún conserva el perfume floral que deja al irse."

    "SCOTT se limpia las manos con un trapo manchado de óleo, suspira y observa su obra: un retrato inacabado, lleno de gestos suaves y miradas ausentes."

    "De pronto, un golpe seco en la puerta lo sobresalta."
    "Otro golpe, más fuerte."

    sc "¿Quién es?"

    "Nadie responde. Solo el eco de los nudillos repitiendo el golpe, esta vez acompañado por un tono de voz autoritario:"

    soldado "¡Abra en nombre del régimen!"

    "SCOTT se queda inmóvil. El trapo cae de sus manos. La pintura fresca gotea desde el pincel hasta el suelo."

    sc "(En voz baja) No... ¿cómo me encontraron...?"

    "Corre hacia un estante, abre una caja oculta tras los libros: el viejo ejemplar de tapas desvanecidas está allí."
    "Lo toma con desesperación, buscando dónde esconderlo."

    "Pero antes de hacerlo, la puerta se abre de golpe. Dos hombres uniformados irrumpen, seguidos por un oficial de mirada fría."

    of "Scott. Está bajo arresto por posesión de material prohibido."

    "SCOTT no responde. Solo aprieta el libro contra el pecho, comprendiendo."
    "Entre el ruido de las botas y las órdenes secas, una idea le atraviesa la mente como un cuchillo: alguien habló."

    "La cámara se queda fija en su rostro, mientras es arrastrado fuera del estudio."
    "El lienzo inacabado queda allí, temblando con el viento que entra por la ventana rota."

    return
