# escena_opcion1.rpy

label esc_7B_1:

    scene bg estudioNoche with fade
    show sc1 at left
    sc "Mira, Tessie... no te atormentes más. Ven, siéntate un momento junto a la ventana. Respirá el aire, aunque huela a óxido. Fumá un poco si eso te ayuda."

    "Tessie se sienta en el alféizar. La brasa del cigarrillo ilumina su rostro, temblando con el viento."

    sc "Solo necesitás dejar que el mundo se aquiete un rato. El tabaco y la luz bastarán por ahora... no curan, pero sirven."

    "Ella asiente en silencio. El humo se mezcla con la bruma gris que entra por la ventana."

    "Scott vuelve al caballete, aunque sus pinceles permanecen inmóviles. El silencio llena el estudio."

    scene bg estudioNoche
    with dissolve

    "Scott observa cómo Tessie se aleja hacia la ventana, su silueta recortada contra la luz mortecina."

    "Se acerca al ventanal… y se detiene."

    "A través del vidrio, en el atrio de la iglesia, el agente sigue allí. Inmóvil."

    "La figura uniforme permanece observando la plaza, rodeada por la bruma como si respirara con ella."

    "Una punzada de repulsión atraviesa a Scott. El aire se vuelve pesado."

    "Por un instante siente que el agente lo mira directamente. No con los ojos… sino con una presencia."

    "Baja la cortina lentamente, intentando sofocar la nausea."

    "Pero incluso con los ojos cerrados, Scott sabe que el agente sigue ahí. Esperándolo."

    return